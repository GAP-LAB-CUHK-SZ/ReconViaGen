# Read Arguments
TEMP=`getopt -o h --long help,new-env,basic,xformers,flash-attn,cumesh,o-voxel,flexgemm,nvdiffrec,spconv,mipgaussian,kaolin,nvdiffrast,demo -n 'setup.sh' -- "$@"`
...
if [ "$NEW_ENV" = true ] ; then
    conda create -n reconviagen_v05 python=3.10
    conda activate reconviagen_v05
...
if [ "$BASIC" = true ] ; then
    pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh open3d xatlas pyvista pymeshfix igraph lpips kornia==0.8.2 huggingface_hub==0.33.4 transformers==5.3.0
...
if [ "$DEMO" = true ] ; then
    pip install gradio==6.0.1 huggingface_hub==1.7.2
fi